...
- **오전 감시 시작 시각 필터링(09:40 KST) 보완**:
  ...
  - **Smart Silence 검증 토큰 불일치 및 다중 수신자 Ack 검증 버그 긴급 패치**:
    - 비상 알림 수신 후 [확인/OK]를 눌렀음에도 Smart Silence가 작동하지 않고 10분마다 비상 알람이 반복 발송되던 문제 원인 규명:
      1) `check_pushover_ack` 호출 시 생성 토큰(`PUSHOVER_TOKEN_EMERGENCY`)과 다른 기본 토큰을 전달하던 불일치 버그 수정.
...
### 2026-08-19
- **조기 출근 긴급 알람 발생 시 동적 출근 시각(11:00) 및 조기 브리핑 자동 전환 구현**:
  - 오전 조기 예약 감지로 인해 긴급 사이렌 알람이 발생한 날은 냔냐님의 출근 시간이 11:00으로 당겨지는 특성을 반영하여, 시스템이 당일 긴급 알람 이력(`morning_alarm:{YYYY-MM-DD}`)을 감지하면 출근 기준 시각을 평상시 `11:20`에서 `11:00`(`EARLY_SHIFT_START_TIME`)으로 동적 조정하도록 구현 완료.
  - **11:00 조기 브리핑 & 근무 모드 진입**: 긴급 알람 발생일의 경우 11:00 정각 실행 시점에 `Mode 2(근무 모드)`로 즉시 전환되어 `📋 [오늘의 조기 출근 브리핑]` 알림이 발송되고, 이후 18:00 퇴근까지 실시간 예약 변동(Diffing) 감시가 원활히 이어지도록 파이프라인 개편 완료.
  - `src/config.py`, `src/main.py`, `.env.example`, `ARCHITECTURE.md`에 동적 조기 출근 설정 상수 및 다이어그램 최신화 반영 완료.
