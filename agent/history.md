...
- **오전 감시 시작 시각 필터링(09:40 KST) 보완**:
  ...
  - **Pushover 5종 전용 토큰 분류 및 퇴근 마감 알림 통합 구현**:
    - Pushover 상단 프로필 아이콘을 알림 상황별로 분리할 수 있도록 5종 전용 토큰(`PUSHOVER_TOKEN_EMERGENCY`, `BRIEFING`, `DIFF_PLUS`, `DIFF_MINUS`, `END`) 파싱 및 미설정 시 기본 `PUSHOVER_API_TOKEN` 자동 폴백 로직을 `src/config.py` 및 `src/notifier.py`에 적용 완료.
    - 퇴근 정각 시각(월 16:00 / 화·수 18:00)에 `🏁 [오늘의 퇴근 마감]` 인사 메시지 및 마지막 18시 변동 내역(Diff) 병합 알림(`alarm_type="end"`) 전송 기능 구축 완료.
