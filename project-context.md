...
3. **알람 만료 시간(EXPIRE) 제약 규칙**:
   - Pushover의 중복 알람 겹침 현상을 방지하기 위해, `PUSHOVER_EXPIRE`(알람 만료 시간)는 항상 GitHub Actions 감시 주기(현재 10분)보다 반드시 짧게 설정해야 합니다.
   - **상세 메커니즘**: 만약 09:50 감시에서 예약 완료 건이 발견되어 긴급 알람이 발생하였으나 냔냐님이 알람을 확인(Acknowledge)하지 못해 알람이 5분 이상 지속되고 있다고 가정해 봅시다. 이때 만료 시간이 감시 주기인 10분보다 길게 설정되어 있다면, 10:00 감시 시점에 동일한 예약(또는 새로운 예약)에 대해 새로운 긴급 알람이 재차 울리며 스마트폰상에서 두 개의 긴급 알람이 중복으로 겹쳐 활성화되는 문제가 생깁니다.
   - **해결책**: 따라서 알람 만료 시간(`PUSHOVER_EXPIRE`)은 반드시 감시 주기(10분 = 600초)보다 짧은 값(예: 300초 = 5분)으로 설정하여, 다음 감시 주기가 시작되기 전에 이전 알람이 자동으로 만료되어 사라지도록 함으로써 중복 겹침 현상을 원천 방지합니다. (로컬 기본값: 300초)
